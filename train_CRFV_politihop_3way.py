...
dir_path = Path('outputs/CRFV_politihop_3way/')
    ...
    for path in dev_data_list:
        ...
        with torch.no_grad():
            for index, data in enumerate(dev_tqdm_iterator):
                ...
                outputs, outputs1, outputs2, outputs3, outputs4, outputs5 = model(data)  # probability [batch,6,6]

                correct = correct_prediction(outputs, labels)
                correct_pred += correct

        dev_accuracy = correct_pred / len(dev_data)
        logger.info('%s Dev total acc: %lf\r\n' % (path, dev_accuracy))
...
feature_num = 768
model = CRFV(feature_num, nclass, max_length, beam_size, args.evi_num, causal_method)
...
for epoch in range(args.epochs):
    ...
    for index, data in enumerate(train_tqdm_iterator):
        ...
        loss1 = F.cross_entropy(outputs2, labels)
        loss2 = F.cross_entropy(outputs3, labels)
